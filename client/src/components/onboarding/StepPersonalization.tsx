...
export default function StepPersonalization({ preferences, onChange, onNext, onBack, errors }: Props) {
    ...
    return (
        ...
        >
            <h2 className="text-2xl font-bold text-foreground mb-1 tracking-tight shrink-0">Cá nhân hóa</h2>
            <p className="text-foreground/40 text-sm mb-6 shrink-0">Giúp chúng tôi hiểu bạn hơn</p>
                        ...
                        {SPORTS.map((s: any) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <span className={`text-sm font-bold ${selected ? 'text-emerald-300' : 'text-foreground/60'}`}>
                                        {s.label}
                                    </span>
                                    {selected && (
                                        <motion.div
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex-1">
                                        <div className={`font-semibold text-sm ${selected ? 'text-emerald-300' : 'text-foreground/70'}`}>
                                            {s.label}
                                        </div>
                                        <div className="text-xs text-foreground/30">{s.desc}</div>
                                    </div>
                                    {selected && (
                                ...
                                >
                                    {filteredDistricts.map((d: string) => (
                                        <button key={d}
                                            ...
                                            className="w-full px-4 py-3 text-left text-sm text-foreground/70 hover:bg-emerald-500/10 hover:text-emerald-300 transition-colors first:rounded-t-xl last:rounded-b-xl"
                                        >
                                            {d}
                                        </button>
                                    ))}
            ...
            <div className="flex gap-3 mt-auto pt-8 pb-2 shrink-0">
                <motion.button onClick={onBack} whileTap={{ scale: 0.97 }}
                    className="flex-1 py-3.5 rounded-xl border border-border text-foreground/60 font-medium hover:border-border transition-all flex items-center justify-center gap-2">
