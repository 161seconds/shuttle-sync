            ...
            <div className="max-w-lg mx-auto px-5 py-6 space-y-4">
                {loading ? (
                    Array.from({ length: 3 }).map((_, i) => (
                        <div key={i} className={`h-28 rounded-2xl ${t.bg.card} border border-border animate-pulse`} />
                    ))
                ) : courts.length === 0 ? (
                    <div className="flex flex-col items-center py-20">
                        ...
                        <p className={`text-sm text-muted-foreground`}>Nhấn ❤️ trên sân để lưu vào đây</p>
                    </div>
                ) : (
                    courts.map(court => (
                        <div key={court._id}
                            ...
                            <div className="flex-1 min-w-0 flex flex-col justify-between py-1 cursor-pointer"
                                ...
                                <div>
                                    <h3 className={`font-bold text-sm text-foreground truncate group-hover:text-emerald-400 transition-colors`}>{court.name}</h3>
