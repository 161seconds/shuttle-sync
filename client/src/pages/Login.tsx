...
export default function Login() {
    ...
    const handleGoogleSuccess = async (credentialResponse: any) => {
        try {
            ...
            if (res.data.success === false || res.data.status === 'error' || !res.data.data?.accessToken) {
                setApiError(res.data.message || 'Lỗi xác thực từ máy chủ');
                return;
            }
            ...
            setApiError(error.response?.data?.message || 'Không thể kết nối với Google. Vui lòng thử lại!');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className={`h-screen overflow-hidden ${DS.bg.base} flex`}>
            ...
            <div className="flex-1 flex items-center justify-center px-6 py-12 relative overflow-y-auto">
                ...
                <div className="w-full max-w-sm relative z-10">
                    ...
                    {/* NÚT GOOGLE CHÍNH CHỦ CỦA SẾP Ở ĐÂY */}
                        ...
                        {GOOGLE_CLIENT_ID ? (
                            <GoogleOAuthProvider clientId={GOOGLE_CLIENT_ID}>
                                <GoogleLogin
                                    ...
                                    onError={() => {
                                        setApiError('Không thể kết nối với cửa sổ Google');
                                    }}
                                    theme="filled_black"
                                    shape="pill"
                                    size="large"
                            ...
                            <div className="w-full p-3 rounded-xl bg-red-500/10 text-red-400 text-xs text-center border border-red-500/20">
                                Thiếu KEY Google Client ID trong file .env
                            </div>
                        )}
                    </div>
