...
export default function MessageInput({ onSend, disabled, replyingTo, onCancelReply }: MessageInputProps) {
    ...
    return (
        <div className="p-4 bg-card border-t border-border">
            {replyingTo && (
                <div className="flex items-center justify-between bg-card p-2 px-4 rounded-t-2xl border-t border-l border-r border-border mb-0 relative">
                    <div className="flex flex-col flex-1 min-w-0 pr-2 border-l-2 border-emerald-500 pl-2">
                        ...
                        <span className="text-sm text-muted-foreground truncate">{replyingTo.content}</span>
                    </div>
                    <button 
                        type="button" 
            ...
            <form onSubmit={handleSubmit} className={`flex items-end gap-3 w-full ${replyingTo ? 'mt-0 border-t border-border' : ''}`}>
                {/* Hidden File Inputs */}
                ...
                <div className="flex gap-2 mb-1">
                    <button
                        ...
                        className="p-2 text-muted-foreground hover:text-emerald-400 hover:bg-emerald-500/10 rounded-full transition-colors"
                        disabled={disabled}
                    >
                ...
                <div className="flex-1 flex flex-col">
                    ...
                    <div className={`bg-card border border-border ${selectedFile ? 'rounded-b-3xl rounded-tr-3xl' : 'rounded-3xl'} flex items-center focus-within:border-emerald-500/50 focus-within:ring-1 focus-within:ring-emerald-500/50 transition-all relative`}>
                        <textarea
                            ref={textareaRef}
                            value={text}
                            ...
                            disabled={disabled}
                            className="w-full max-h-32 min-h-[44px] py-3 px-4 bg-transparent text-sm text-foreground resize-none focus:outline-none custom-scrollbar my-auto"
                            rows={1}
                            maxLength={500}
                            onKeyDown={(e) => {
                        ...
                        <div className="flex items-center gap-1 pr-2 relative" ref={quickRef}>
                            {text.length > 0 && (
                                <span className={`text-[10px] ${text.length >= 450 ? 'text-red-400 font-bold' : 'text-muted-foreground'} transition-colors`}>
                                    {text.length}/500
                                </span>
                            )}
                            <button
                                ...
                                className={`p-2 rounded-full transition-colors ${showQuick ? 'bg-emerald-500/10 text-emerald-400' : 'text-muted-foreground hover:text-emerald-400 hover:bg-emerald-500/10'}`}
                                disabled={disabled}
                            >
                            ...
                            <AnimatePresence>
                                {showQuick && (
                                    <motion.div
                                        ...
                                        className="absolute bottom-full right-0 mb-3 w-64 bg-card border border-border rounded-2xl shadow-xl overflow-hidden z-50"
                                    >
                                        <div className="p-3 border-b border-border">
                                            <div className="text-[10px] font-bold text-muted-foreground uppercase mb-2">Emoji nhanh</div>
                                        ...
                                        <div className="p-2 max-h-40 overflow-y-auto custom-scrollbar">
                                            <div className="text-[10px] font-bold text-muted-foreground uppercase mb-1 px-1 mt-1">Tin nhắn mẫu</div>
                                            {QUICK_MESSAGES.map(msg => (
                                                <button
                                                    key={msg}
                                                    type="button"
                                                    onClick={() => { setText(prev => (prev ? prev + ' ' : '') + msg); setShowQuick(false); textareaRef.current?.focus(); }}
                                                    className="w-full text-left px-3 py-2 text-sm text-muted-foreground hover:text-foreground hover:bg-muted rounded-xl transition-colors"
                                                >
                                                    {msg}
                                                </button>
                                            ))}
