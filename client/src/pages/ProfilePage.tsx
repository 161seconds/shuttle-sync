...
const MENU = [
    ...
    { icon: <Bookmark className="w-4 h-4" />, label: 'Sân yêu thích', badge: '3' },
    { icon: <History className="w-4 h-4" />, label: 'Lịch sử đặt sân', badge: '12' },
    { icon: <Award className="w-4 h-4" />, label: 'Giải đấu của tôi', badge: null },
    { icon: <Users className="w-4 h-4" />, label: 'Nhóm chơi', badge: '2' },
    { icon: <Bell className="w-4 h-4" />, label: 'Thông báo', badge: '5' },
    ...
    return (
        <div className="max-w-lg mx-auto px-4 pb-24 md:pb-8 py-6">
            ...
            <div className={`${t.bg.card} rounded-2xl border ${t.border.subtle} p-6 mb-6 text-center`}>
                ...
                <h2 className={`text-lg font-bold ${t.text.primary}`}>Lê Minh Player</h2>
                <p className={`text-xs ${t.text.muted} mt-0.5`}>user1@shuttlesync.vn</p>

                <div className="grid grid-cols-3 gap-3 mt-5">
                    {[
                        { label: 'Đã đặt', value: '12' },
                        { label: 'Nhóm chơi', value: '5' },
            ...
            {/* Logout */}
            <button className="w-full flex items-center gap-4 px-4 py-3.5 rounded-xl mt-4 hover:bg-red-500/5 transition-colors">
